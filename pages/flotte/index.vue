...
import { MsBadge, MsCard, MsPageHero, MsCtaBanner } from '@meridian-synergy/ui'

const { t, locale } = useI18n()
const localePath = useLocalePath()

useSeoMeta({
  title: () => t('fleetPage.meta.title'),
  description: () => t('fleetPage.meta.description'),
})
...
useSchemaOrg([
  defineBreadcrumb({
    itemListElement: [
      ...
      { name: t('nav.drones'), item: `https://meridian-synergy.com${locale.value === 'en' ? '/en' : ''}/drones` },
      { name: t('nav.fleet') },
    ],
  }),
])
...
const { data: drones } = await useAsyncData(`fleet-${locale.value}`, async () => {
...
<template>
  <div>
    <MsPageHero
      :crumbs="[{ label: t('breadcrumb.home'), href: localePath('/') }, { label: t('nav.drones'), href: localePath('/drones') }, { label: t('nav.fleet') }]"
      :badge="t('fleetPage.hero.badge')"
      :title="t('fleetPage.hero.title')"
      :desc="t('fleetPage.hero.desc')"
    />
      ...
      <div class="container">
        <div class="drones-grid">
          <NuxtLink
            ...
            :key="drone.slug"
            :to="localePath(`/flotte/${drone.slug}`)"
            class="drone-link"
          >
            <MsCard>
    ...
    <!-- CTA -->
    <MsCtaBanner
      :title="t('fleetPage.contact.title')"
      :desc="t('fleetPage.contact.desc')"
      :label="t('fleetPage.contact.btn')"
